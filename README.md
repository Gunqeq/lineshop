...
## Deploy ขึ้นจริง (ไม่ต้องเปิดเครื่องตัวเองทิ้งไว้)

เมื่อพร้อม deploy แนะนำ:
- **Railway** หรือ **Render** — ใช้งานง่าย มี free tier เริ่มต้น
- ตั้งค่า environment variables ในระบบของแพลตฟอร์มเหมือนกับไฟล์ .env
- เปลี่ยน Webhook URL ใน LINE Console เป็น URL จริงของ deploy

## สิ่งที่ควรทำต่อ (หลัง MVP นี้ใช้งานได้แล้ว)

ตามที่คุยกันไว้ ฟีเจอร์ที่ยังไม่ต้องทำตอนนี้ เก็บไว้ภายหลัง:
- ย้ายข้อมูลสินค้าจาก `store_data.py` ไปเป็น database จริง พร้อมหน้า admin ให้ร้านค้าแก้เอง
- เชื่อมระบบสต็อกเรียลไทม์
- รองรับ Facebook Messenger เพิ่ม
- Dashboard สถิติคำถามที่ลูกค้าถามบ่อย
